...
**Важно (Supabase UI):** экран **иконка «Database» (цилиндр) → Settings** — только пул/SSL/пароль, **без** готовой строки `postgresql://…`.

**Где взять URI (официально, на всех версиях UI):** на **главной странице проекта** (Overview / Home) нажми **Connect** (часто справа сверху) — в модалке вкладки вроде **ORM**, **App frameworks**, **URI**; выбери **Session pooler** или **Transaction** (порт **6543**, хост `*.pooler.supabase.com`) и скопируй строку.

**Если пункта «Database» в боковом меню Project settings у тебя нет** — не ищи его: у части аккаунтов/раскладок Supabase его убрали или переименовали; **Connect на главной проекта** — основной путь. Дополнительно: строка поиска по дашборду → набери `connection string`.

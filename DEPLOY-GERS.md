...
2. **Регистрация:** `/register` — имя, **логин** (латиница, цифры, `_`), пароль (не короче 8 символов). Почта не спрашивается; в БД сохраняется служебный адрес вида `{логин}@users.local`. Новые пользователи по умолчанию **PENDING**, пока админ не активирует.
